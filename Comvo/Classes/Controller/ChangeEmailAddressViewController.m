...
- (IBAction)onTouchBtnSubmit: (id)sender {
    ...
    NSString *newEmail = mInputEmailAddress.text;
    ...
    void ( ^successed )( id responseObject ) = ^( id responseObject ) {
        ...
        if (result) {
            ...
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle: @"Email Address has been changed successfully." message: [responseObject valueForKey: @"Success"] delegate: nil cancelButtonTitle: @"Ok" otherButtonTitles: nil];
            [alertView show];
        }
        else {
    ...
    NSString *updatelist = [[NSString alloc] initWithData:[NSJSONSerialization dataWithJSONObject:@{@"email":newEmail} options:0 error:nil] encoding:NSUTF8StringEncoding];
